            ...
            >
                ...
                {technicians.filter(t => t.latitude && t.longitude).map(tech => (
                    <React.Fragment key={tech.id}>
                        {/* Connecting Line from User to Craftsman */}
                        {userLocation && (
                            <Polyline 
                                positions={[
                                    [userLocation.lat, userLocation.lng], 
                                ...
                                pathOptions={{ color: '#9ca3af', weight: 2, dashArray: '5, 5', opacity: 0.7 }}
                            />
                        )}
                        ...
                        <CircleMarker 
                            ...
                            radius={15}
                        >
                            <Tooltip direction="top" offset={[0, -10]} opacity={1} permanent className="distance-tooltip">
                                {tech.distance !== undefined ? (
                                    <span>{Number(tech.distance).toFixed(1)} كم</span>
                                ) : (
                                    <span>{tech.name}</span>
                                )}
                            </Tooltip>
